---
title: Dr. Aneta Koseska
date: 2024-01-01
type: collaborator

# Collaborator info
name: Dr. Aneta Koseska
organization: Cellular computations and learning Group, Max Planck Institute for neurobiology of behavior — caesar, Bonn, Germany
role: Group Leader
country: 🇩🇪

# Avatar image (place avatar.jpg/png in this folder)
avatar: avatar.png

# Links to related projects/publications
links:
  - name: "MPI-NB Profile"
    url: "https://mpinb.mpg.de/en/research-groups/groups/cellular-computations-and-learning/research-focus-ccl-eng.html"
  - name: "Google Scholar Profile"
    url: "https://scholar.google.com/citations?user=3ePo_TkAAAAJ&hl=de"
    icon: academicons/google-scholar

# Tags for filtering
tags:
  - Nonlinear Dynamics
  - Cell Systems
  - Criticality
---

<div style="display: flex; align-items: flex-start; gap: 10px;">
  <div style="margin-top: -2.0em;">
    {{< figure src="avatar.png" alt="Prof. Sudeshna Sinha" class="avatar" width="600" height="600" >}}
  </div>
  <div>
    <a href="https://mpinb.mpg.de/en/research-groups/groups/cellular-computations-and-learning/ccl-group-leader-de.html">Dr. Aneta Koseska</a> is a distinguished researcher in the field of nonlinear dynamics, complex systems and cellular dynamics.
    <br>
    Our collaboration focused on building a theory of biochemical information processing with transients to under "How cells process dynamic information in real-time?"

  </div>
</div>

## Collaborated projects with Prof. M. Stender:

- **2025:** [Homeorhetic regulation of cellular phenotype](/publications/homeorhetic-regulation-2024). This article is now available on [BioArxiv, read full article here](https://scholar.google.com/citations?view_op=view_citation&hl=en&user=X6MgHskAAAAJ&sortby=pubdate&citation_for_view=X6MgHskAAAAJ:J_g5lzvAfSwC).
- **2023:** [PhD thesis](https://scholar.google.com/citations?view_op=view_citation&hl=en&user=X6MgHskAAAAJ&sortby=pubdate&citation_for_view=X6MgHskAAAAJ:YFjsv_pBGBYC) on building a *Theory of biochemical information processing with transients*.
 
---